...
# Define paths for Firecracker binary and kernel
ARCH="$(uname -m)"
...
# Download Firecracker binary if not already present
...
# Download the Firecracker-optimized Linux kernel if not already present
if [ ! -f "$KERNEL_BINARY" ]; then
    log "Downloading Firecracker-optimized Linux kernel..."
    latest_kernel=$(curl -s ${KERNEL_URL} | grep -oP 'firecracker-ci/v1.10/x86_64/vmlinux-5\.10\.\d{3}' | head -n 1)
    if [ -z "$latest_kernel" ]; then
        handle_error "Failed to find the latest kernel version."
    fi
    curl -L "https://s3.amazonaws.com/spec.ccfc.min/${latest_kernel}" -o "$KERNEL_BINARY" || handle_error "Failed to download the Linux kernel."
else
    log "Kernel binary already exists. Skipping download."
fi

# Verify KVM support
log "Checking for KVM support..."
if kvm-ok; then
    log "KVM is correctly set up. Skipping KVM configuration."
else
    log "Setting up KVM permissions..."
    sudo modprobe kvm
